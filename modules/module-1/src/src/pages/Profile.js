...
// const registerUrl = "https://108u32xo9b.execute-api.us-east-1.amazonaws.com/data/change-password";
...
export default function Profile() {
  ...
  // const adminEmail = user !== 'undefined' && user ? user.admin : '';
  ...
  const handleOpen = (event) => {
    console.log("Clicked handeOpen", event);
  }
  ...
  function saveEditChanges(editName, editUsername, editPhone, editCountry, editAddress, editSecretQuestion, editSecretAnswer, email) {
    ...
    .then(() => {
      // console.log("In response condition", response)
      setMessage("User details updated");
      setSuccess(true);

      // setEditName(editName);
      // console.log(response.data.body.username)
      // setEditUsername(response.data.body.username);
      // setEditPhone(editPhone);
      // setEditCountry(editCountry);
      // setEditAddress(editAddress);
      // setEditSecretQuestion(editSecretQuestion);
      // setEditSecretAnswer(editSecretAnswer);
      })
      .catch((error) => {
        if (error.response.status === 401 || error.response.status === 403) {
          // console.log("In if condition", error)
          // console.log("In if condition", error.response)
          setMessage(error.response.data.body);
          setSuccess(true);
        } else {
          // console.log("In else condition", error)
          // console.log("In else condition", error.response)
          setMessage('There is something wrong in the backend server');
          setSuccess(true);
        }
      });
