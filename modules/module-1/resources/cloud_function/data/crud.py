...
def create(firestore_collection, value, document=None, merge_values=True):
    try:
        ...
        if document == None:
            ...
            client.collection(firestore_collection).document(document).set(
                value, merge=merge_values
            )
        return True
                ...
                if len(condition) == 7:
                    ...
                    for part in parts:
                        docs = (
                            ...
                            .where(condition[4], condition[5], part)
                            .stream()
                        )
                        for doc in docs:
...
def update(firestore_collection, value, ids, full_overrite=False):
    ...
    try:
        ...
        if full_overrite == False:
            docs = (
                client.collection(firestore_collection).where("id", "==", ids).stream()
            )
            for doc in docs:
...
def delete(
    ...
    document=None,
    value=None,
    document_contaion_sub_collection=False,
):
    try:
        client = firestore.Client()
        ...
        if kind == "field":
            if value != None:
                if ids == None:
                    client.collection(firestore_collection).document(document).update(
                        {value: firestore.DELETE_FIELD}
                    )
                else:
                    docs = (
                        client.collection(firestore_collection)
                    ...
                    for doc in docs:
                        client.collection(firestore_collection).document(doc.id).update(
                            {value: firestore.DELETE_FIELD}
                        )
            else:
                print("Provide value")
