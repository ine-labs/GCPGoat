...
def generateResponse(statusCode, body):
    ...
    return Response(body, headers=headers, status=statusCode)
...
def upload_file(img_data, bucket_name, url=None):
    ...
    try:
        destination_blob_name = (
            "images/" + datetime.utcnow().strftime("%Y%m%d%H%M%S%f") + ".png"
        )
        blob = bucket.blob(destination_blob_name)
        blob.upload_from_string(file)
        url = "https://storage.googleapis.com/%(mybucket)s/%(file)s" % {
...
def main(req):
    ...
    if method == "POST" and path == "register":
        ...
        if not bcrypt.checkpw(
            (user_password).encode("utf-8"), (db_user[0]["password"]).encode("utf-8")
        ):
        ...
        responses = {"user": userInfo, "token": (gen_token).decode("utf-8")}
        ...
        if (
            user_data[0]["secretQuestion"] != secretQuestion
            or user_data[0]["secretAnswer"] != secretAnswer
        ):
            ...
            for field in required_info:
                if field not in new_user_data:
                    return generateResponse(
                        200, json.dumps({"body": "All Fields required"})
                    )
